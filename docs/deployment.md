...
应用需要火山引擎 Ark API 密钥，**不会**随部署脚本自动上传（避免泄露）。
...
ARK_MODEL=你的-model-或-endpoint-id
EOF
```

参考本地 `.env.local.example`。
...
检查服务器 `/opt/vibe-live/.env` 中 `ARK_API_KEY`、`ARK_MODEL` 是否正确，然后：
...
| 8080 | 后端 API | **不要用**，会冲突 |
| 3000 | 未占用 | Next.js 默认端口，推荐 |

vibe-live 是 **Next.js 全栈应用**：页面和 `/api/generate` 都在同一个 3000 进程里，不像 flowdraft 那样前后端分离。因此反代时要把**整站**指到 3000，不能把 `/api` 再拆到 8080。
...
此时 3000 只需本机访问，**安全组不必对公网开放 3000**。

### 不要做的事

- 不要把 vibe-live 绑到 **8080**（flowdraft 后端已占用）
- 不要改 flowdraft 的 `location /api` 指向 3000（会破坏 flowdraft）
- 不要用 `default_server` 抢 flowdraft 的 80 端口

## 可选：绑定域名 + HTTPS

若已有 Nginx + Certbot，优先用上方**方案 B（子域名）**。无子域名时，也可为独立域名新建 `server` 块，`proxy_pass http://127.0.0.1:3000` 即可。
